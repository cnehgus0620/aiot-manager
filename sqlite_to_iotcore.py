...
- S3/Glue 파티션 구조: date(yyyy-MM-dd) / hour(HH) / room_p
...
def calc_std(avg, sumsq, n):
    ...
    return round(var ** 0.5, 4) if var >= 0 else 0
...
def publish_window(client, start_utc, end_utc, conn_ckpt):
    ...
    for r in rows:
        ...
        payload = {
            ...
            "room_p": room_p   # ✅ Firehose Dynamic Partition Key
        }

        msg = json.dumps(payload, ensure_ascii=False)
        client.publish(TOPIC, msg, QOS)
        log(f"[PUBLISH] {r['dev']} {room_p} ({start_s}~{end_s})")
